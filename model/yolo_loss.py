...
def smooth_BCE(eps=0.1):
    # 为了解决正负样本不平衡的问题，通常会在交叉熵损失的前面加上一个调整系数阿尔法
    # 正负样本不平衡的调整参数
...
class YoloLoss(nn.Module):
    def __init__(self,class_scale,image_size,gr,is_ebr=True,is_fl=True):
    ...
    def forward(self,x,y=None,c=None):
        ...
        off=torch.tensor(
            [[0,0],
             ...
             [-1,1],
             [-1,-1],
             [1,1]],device=device).float()*g

        for i,x_i in enumerate(x):
            cfg=c[i]
            scale_x_y=float(cfg['scale_x_y'])
            grid_size=int(cfg['grid_size'])
            # 找出合适的gt类别号，gtbox, 选取预测box的索引，锚框
            tcls,tbox,indices,anchors=self.build_targets(y,grid_size,cfg,device,gain,nt,g,off)

            # 候选框索引: 图片索引，anchor索引，y网格索引，x网格索引,均为m维向量[m]
            b,a,gj,gi=indices
            # 定义置信度，shape与预测tensor一样,(input,full_value: 0.1) 64x3x13x13
            tobj=torch.full_like((x_i[...,0]),self.cn,device=device)

            # 选取候选框数量 m，这么写比len(b)的鲁棒性更高
            n=b.shape[0]
            iou=torch.zeros(1,device=device)
            # 如果有目标存在，计算lcls和lbox，否则全是背景，不计算，但lobj是针对整个检测图的，所以始终要计算
            if n:
                # 获取正样本预测box的信息，shape[m,25] conf,x,y,w,h,cls...
                ps=x_i[b,a,gj,gi]
                # sigmoidx2-0.5,消除网格敏感
                pxy=ps[:,1:3].sigmoid()*scale_x_y-(scale_x_y-1.)/2.
                # tw[m,2]*pw[m,2]
                ...
                if nc>1:
                    t=torch.full_like(ps[:,5:],self.cn,device=device)  # [m,20]
                    # range(m个box),即逐行中的指定列进行赋值为0.9，其余19列值保持为0.1
                    t[range(n),tcls]=self.cp
                    #分类是针对box的，所以shape mx5
                    lcls+=self.BCEcls(ps[:,5:],t,self.cp)   #未乘以分类乘数

            # 置信度是针对整个检测图的所有网格的，所以shape 64x3x13x13
            lobj+=self.BCEobj(x_i[...,0],tobj)*float(cfg['obj_normalizer'])  #不同检测图的置信度损失权重
    ...
    def build_targets(self,y,grid_size,cfg,device,gain,nt,g,off):
        mask = cfg['mask'].split(',')
        ...
        anchors=torch.tensor(anchors[mask],dtype=torch.float32,device=device)/self.image_size*grid_size
        ...
        if nt:
            ...
            #选出那些gt和anchor的尺寸比小于四倍的那些gt框，shape[n,7]
            t=t[j]
